...
namespace fmt
{
	template <>  struct formatter<hipError_t>
	{
		// No custom formatting options, so `parse` remains unchanged
		constexpr auto parse(format_parse_context &ctx) -> decltype(ctx.begin()) {
		return ctx.end();
		}

		// Important: Mark `format` method as `const`
		template <typename FormatContext>
		auto format(const hipError_t &err, FormatContext &ctx) const -> decltype(ctx.out()) {
		const char* err_str = hipGetErrorString(err);
		return format_to(ctx.out(), "{}", err_str ? err_str : "Unknown HIP error");
		}
	};
